...
import com.ericsson.cifwk.taf.metrics.queue.AmqpConsumer;
...
//        startProducer();
    ...
    private void startConsumer() throws Exception {
        ...
        AmqpConsumer amqpConsumer = new AmqpConsumer(new Kryo(), amqp, dbWriter);
        amqpConsumer.start();
    }
    ...
    private void startProducer() throws Exception {
        ...
        try (Dumpling dumpling = AmqpDumpling.create(amqp)) {
            dumpling.initialize();
            HttpMetricsEmulator.emulate(dumpling, count);
        }
    }
}
