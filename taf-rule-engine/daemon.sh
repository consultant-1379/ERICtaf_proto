...
serviceNameLo="taf-dashboard"                          # service name with the first letter in lowercase
serviceName="taf-dashboard"                            # service name
...
javaArgs="-Xms256m -Xmx512m -server -jar $applDir/taf-dashboard-1.0-SNAPSHOT.jar"                     # arguments for Java launcher
javaCommandLine="$javaExe $javaArgs"                   # command line to start the Java service application
javaCommandLineKeyword="taf-dashboard.jar"             # a keyword that occurs on the commandline, used to detect an already running service process and to distinguish it from others
